...
function M.keymaps()
  ...
  group = "General"
  dofile(root .. "/lua/helloguvim/config/keymaps.lua")
  group = "LSP"
  local lsp = dofile(root .. "/lua/helloguvim/plugins/lsp/keymaps.lua")
  ...
  local core = require("lazy.core.plugin").Spec.new({ import = "helloguvim.plugins" })
  ...
  Util.walk(root .. "/lua/helloguvim/plugins/extras", function(path, name, t)
    if t == "file" and name:find("%.lua$") then
      local modname = path:gsub(".*/lua/", ""):gsub("/", "."):gsub("%.lua$", "")
      local extra_doc = "/plugins/extras/" .. modname:gsub("helloguvim%.plugins%.extras%.", "")
...
```]]):format(
        name,
        Util.read_file(vim.fn.fnamemodify(root .. "/../HelloGuVim-starter/lua/config/" .. name .. ".lua", ":p"))
      ),
      "",
      "</TabItem>",
      ([[<TabItem value="defaults" label="Default %s">]]):format(title),
      "",
      ([[```lua title="helloguvim.config.%s"
%s
```]]):format(name, Util.read_file(vim.fn.fnamemodify(root .. "/lua/helloguvim/config/" .. name .. ".lua", ":p"))),
      "",
      "</TabItem>",
      "</Tabs>",
      "",
...
function M.recipes()
  local src = Util.read_file(vim.fs.normalize("~/projects/helloguvim.github.io/lua/recipes.lua"))
...
function M.update2()
  local docs = vim.fs.normalize("~/projects/helloguvim.github.io/docs")

  local config = Docs.extract("lua/helloguvim/config/init.lua", "\nlocal defaults = ({.-\n})")
  ...
  Docs.save({
    lazy = {
      content = [[```lua title="lua/config/lazy.lua"]] .. "\n" .. Util.read_file(
        vim.fn.fnamemodify(root .. "/../HelloGuVim-starter/lua/config/lazy.lua", ":p")
      ) .. "\n```",
    },
  ...
  Util.walk(root .. "/lua/helloguvim/plugins/extras", function(path, name, type)
...
require("lazy").setup({
  spec = {
    { "helloguvim/helloguvim", import = "helloguvim.plugins" },
    { import = "%s" },
    { import = "plugins" },
  },
})
```
  ...
  local examples = vim.fn.fnamemodify(root .. "/../helloguvim-starter/lua/plugins/example.lua", ":p")
  Docs.save({
...
function M.plugins(path)
  local test = root .. "/lua/helloguvim/plugins/" .. path
